...
const Graph = ({ lastMonth, thisMonth, barColor, textColor, arrow, decrease }) => {
      ...
      <GraphContainer>
        <BarContainer>
          <Bar height={lastMonth} backgroundColor={barColor}>
            <Value fontSize={'45px'} opacity={'50%'} color={textColor}>{formattedLastMonth}</Value>
          </Bar>
        ...
        <BarContainer>
          <Bar height={thisMonth} backgroundColor={barColor}>
            <Value fontSize={'45px'} color={textColor}>{formattedThisMonth}</Value>
          </Bar>
          <Label color={'#49454F'} fontWeight={'800'}>이번달</Label>
        </BarContainer>
      </GraphContainer>
  );
};
