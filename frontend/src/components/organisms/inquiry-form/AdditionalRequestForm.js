...
const AdditionalRequestForm = ({ formData, handleFormDataChange }) => { // 추가요청사항
    ...
    return (
        <div className={Container} style={{ marginTop: "-2vh" }}>
            <div className={Sheet}>
                ...
                {isChecked ? (
                    <div className={Opend}>
                        <textarea className={_TextArea}
                                  placeholder="내용을 입력해 주세요"
                                  wrap="hard"
                                  value={additionalRequests}
                                  onChange={(e) => handleFormDataChange('additionalRequests', e.target.value)}
                        />
                    </div>
                ) : (
                    ''
